...
const get_public_checkin = (args: Args) => {
  ...
  // Central project groups; fall back to the groups members already use.
  let groups = parseGroups(checkin.project_groups);
  if (groups.length === 0) {
    groups = (
      db
        .prepare(
          `select distinct group_name from members
           where project_id = ? and status = 'active' and group_name is not null and trim(group_name) <> ''
           order by group_name`,
        )
        .all(checkin.project_id) as { group_name: string }[]
    ).map((r) => r.group_name);
  }
...
const get_public_registration = (args: Args) => {
  const page = db
    .prepare(
      `select rp.id, rp.is_active, rp.title, rp.description, rp.ask_email, rp.ask_group,
              rp.groups, rp.project_id, p.name as project_name, p.groups as project_groups,
  ...
  // Groups come from the project; legacy per-page lists act as fallback.
  const projectGroups = parseGroups(page.project_groups);
  const groups = projectGroups.length ? projectGroups : parseGroups(page.groups);
